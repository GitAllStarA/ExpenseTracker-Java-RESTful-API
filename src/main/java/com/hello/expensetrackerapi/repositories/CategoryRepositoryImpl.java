...
public class CategoryRepositoryImpl implements CategoryRepository{
    ...
    public static final String SQL_FIND_BY_ID = "SELECT C.CATEGORY_ID, C.USER_ID, C.TITLE, C.DESCRIPTION, " +
            "COALESCE(SUM(T.AMOUNT), 0) TOTAL_EXPENSE " +
            "FROM ET_TRANSACTIONS T RIGHT OUTER JOIN ET_CATEGORIES C ON C.CATEGORY_ID =" +
            "T.CATEGORY_ID WHERE C.USER_ID = ? AND C.CATEGORY_ID = ? GROUP BY C.CATEGORY_ID";
    ...
    private static final String SQL_UPDATE = "UPDATE ET_CATEGORIES SET TITLE = ?, DESCRIPTION = ? WHERE USER_ID =? " +
            "AND CATEGORY_ID = ?";
    ...
    public Category findById(Integer userId, Integer categoryId) throws EtResourceNotFoundException {
        try{
            ...
            throw new EtResourceNotFoundException("Category Requested not found");
        }
    }
